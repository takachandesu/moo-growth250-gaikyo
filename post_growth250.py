...
次に、**ウェブ検索で本日のグロース250の「大引け(取引終了後の終値)」情報を調べてください**:
  - **最優先で「大引け」「終値」を探す**。検索語の例:「グロース250 大引け {today_label}」
    「新興株{today_label} グロース250 終値」。日経「新興株◯日…」や株探・時事通信・
    ウエルスアドバイザー等の大引け記事から、終値・前日比(ポイント)・騰落率(%)・
    方向(反発/続伸/反落/続落)を取得する。
  - **必ず「大引け(終値)」の数値を使うこと。前引け(午前)の数値を終値と取り違えない**。
    例えば「前引けは…835.56」とある記事は午前の値なので、終値としては使わない。
  - 大引けの数値がどうしても見つからない場合に限り、前引けの数値を使い、見出し・本文で
    明確に「前引け時点」と記す。それも無ければ数値は書かず方向感のみ(数字は創作しない)。
